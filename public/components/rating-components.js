...
class RatingComponent extends HTMLElement {
  ...
  attachEventListeners() {
    const ratingInputs = this.shadowRoot.querySelectorAll(
      'input[name="rating"]'
    );
  ...
  saveState() {
    localStorage.setItem(
      ...
      JSON.stringify({ rating: this._ratingValue, review: this._reviewComment })
    );
  }

  loadState() {
    const savedState =
      JSON.parse(localStorage.getItem("ratingComponentState")) || {};
  ...
  if (reviewInput) {
    ...
    try {
      const reviewResponse = await fetch(
        "http://localhost:5001/api/movie/review",
        {
          method: "POST",
          headers: {
      ...
      const updateResponse = await fetch(
        `http://localhost:3000/api/v1/movies/kino/${localStorage.getItem(
          "movieId"
        )}`,
        {
          method: "PUT",
